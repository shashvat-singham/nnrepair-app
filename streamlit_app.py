...
availability = pd.DataFrame(
    [
        ...
        {
            "Artifact": "Z3 solutions (290 files)",
            "Size": "14 MB",
            "Available": "Yes" if have_z3_solutions() else "Local only",
            "Used by": "Solver Output",
        },
        {
            "Artifact": "Extracted weights & datasets",
            "Size": "954 MB",
            "Available": "Yes" if have_weights() else "Local only",
...
st.caption(
    "The weight dumps and MNIST/CIFAR datasets are too large to host — ten of "
    "the input files are 94 MB each. Clone the repository to enable the pages "
    "that need them."
)

st.markdown(
    f"<p style='color:{INK['muted']};font-size:0.85rem;margin-top:2rem'>"
    "The analysis code is a Python port of the artifact's Java implementation "
    "under <code>NNRepair/CombinationCode</code>. See the repository README for "
